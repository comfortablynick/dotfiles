...
def query_package(module, pkg):
    ...
    if repo_check_rc == 0 and repo_check_stderr == "":
        ...
        # Indicate package is up-to-date,
        # but just because we hit an error contacting the repo
...
def install_packages(module, pkgs, state):
    ...
    if state == "latest" and len(package_err) > 0:
        message = (
            'But could not ensure "latest" state for %s package(s)'
            "as remote version could not be fetched." % package_err
        )
