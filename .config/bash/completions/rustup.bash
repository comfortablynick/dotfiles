_rustup() {
    ...
    case "${cmd}" in
        rustup)
            opts=" -v -h -V  --verbose --help --version   show install uninstall update default toolchain target component override run which doc man self set completions help  docs"
        ...
        rustup__default)
            ...
            ;;
        rustup__doc)
            opts=" -h -V  --path --alloc --book --cargo --core --edition-guide --nomicon --proc_macro --reference --rust-by-example --rustc --rustdoc --std --test --unstable-book --help --version --toolchain  "
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- ${cur}) )
                return 0
            fi
