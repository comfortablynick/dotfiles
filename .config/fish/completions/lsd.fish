...
complete -c lsd -n "__fish_use_subcommand" -l permission -d 'How to display permissions' -r -f -a "rwx octal"
...
complete -c lsd -n "__fish_use_subcommand" -l sort -d 'sort by WORD instead of name' -r -f -a "size time version extension none"
complete -c lsd -n "__fish_use_subcommand" -l group-dirs -d 'Sort the directories then the files' -r -f -a "none first last"
complete -c lsd -n "__fish_use_subcommand" -l blocks -d 'Specify the blocks that will be displayed and in what order' -r -f -a "permission user group context size date name inode links"
...
complete -c lsd -n "__fish_use_subcommand" -l hyperlink -d 'Attach hyperlink to filenames' -r -f -a "always auto never"
...
complete -c lsd -n "__fish_use_subcommand" -s U -l no-sort -d 'Do not sort. List entries in directory order'
complete -c lsd -n "__fish_use_subcommand" -s r -l reverse -d 'Reverse the order of the sort'
complete -c lsd -n "__fish_use_subcommand" -l group-directories-first -d 'Groups the directories at the top before the files. Same as --group-dirs=first'
...
complete -c lsd -n "__fish_use_subcommand" -s Z -l context -d 'Print security context (label) of each file'
complete -c lsd -n "__fish_use_subcommand" -l header -d 'Display block headers'
