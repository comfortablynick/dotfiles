function _edit --description "Edit file using fuzzy finding"
    # Adapted from: https://github.com/PatrickF1/fzf.fish/blob/main/functions/_fzf_search_directory.fish
    ...
    if test $status -eq 0
        ...
        if test (count $file_paths_selected) = 1
            ...
            if test "$commandline_tokens" = "$token" -a -d "$file_paths_selected"
                ...
                # else
                #     fish_commandline_prepend $EDITOR
            end
            commandline -tr -- (string escape -- $file_paths_selected | string join ' ')
        end
    end
