...
local sections = {
  ...
  {
    title = string.format("Recent Files in %s", cur_dir),
    show = current_dir_files(),
  },
  ...
  for _, section in ipairs(sections) do
    if next(section.show) ~= nil then
      ...
      for _, item in ipairs(section.show) do
        ...
        vim.api.nvim_buf_add_highlight(0, -1, "StartifyBracket", linenr + size,
                                       3, 4)
        vim.api.nvim_buf_add_highlight(0, -1, "StartifyNumber", linenr + size,
                                       4, 4 + key_len)
        vim.api.nvim_buf_add_highlight(0, -1, "StartifyBracket", linenr + size,
                                       4 + key_len, 5 + key_len)
...
local function setup_keys()
  -- First, the nav keys
  vim.api.nvim_buf_set_keymap(0, "n", "h", "<NOP>",
                              {noremap = true, silent = true})
  vim.api.nvim_buf_set_keymap(0, "n", "l", "<NOP>",
                              {noremap = true, silent = true})
