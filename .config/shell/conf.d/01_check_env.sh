...
env_sha="$(eval "$sha_cmd" "$HOME/dotfiles/env.toml" | cut -d' ' -f1)"

if [ ! -f "$env_file" ] || [ "$env_sha" != "$(cat "$env_file")" ]; then
    echo "Changes made to env.toml since $CURRENT_SHELL last started."
    "$HOME/.asdf/bin/asdf" exec python3 "$HOME/bin/parse_env" "$HOME/dotfiles/env.toml" "$XDG_CONFIG_HOME/$CURRENT_SHELL/conf.d/01_env.$CURRENT_SHELL" -s "$CURRENT_SHELL"
