...
if [ ! -f "$env_file" ] || [ "$env_sha" != "$(cat "$env_file")" ]; then
    ...
    "$HOME/.asdf/bin/asdf" exec python3 "$HOME/bin/parse_env" "$HOME/dotfiles/dotfiles/env.toml" "$XDG_CONFIG_HOME/$CURRENT_SHELL/conf.d/01_env.$CURRENT_SHELL" -s "$CURRENT_SHELL"
