...
function __fundle_seq -a upto #{{{1
    seq 1 1 $upto 2>/dev/null
end

function __fundle_next_arg -a index #{{{1
    set -l args $argv[2..-1]
...
function __fundle_compare_versions -a version1 -a version2 #{{{1
...
function __fundle_date -d "returns a date" #{{{1
    set -l d (date +%s%N)
...
function __fundle_self_update -d "updates fundle" #{{{1
...
function __fundle_url_rev -d "prints the revision from the url" -a git_url #{{{1
...
function __fundle_remote_url -d "prints the remote url from the full git url" -a git_url #{{{1
    echo $git_url | cut -d '#' -f 1
end

function __fundle_rev_parse -d "prints the revision if any" -a dir -a commitish #{{{1
...
function __fundle_commit_sha -d "returns sha of the commit-ish" -a dir -a commitish #{{{1
    if test -d "$dir/.git"
        set dir "$dir/.git"
    end
...
function __fundle_plugins_dir -d "returns fundle directory" #{{{1
...
function __fundle_no_git -d "check if git is installed" #{{{1
    # `command -q` is >= 2.5.0
...
function __fundle_check_date -d "check date" #{{{1
...
function __fundle_get_url -d "returns the url for the given plugin" -a repo #{{{1
    echo "https://github.com/$repo.git"
end

function __fundle_update_plugin -d "update the given plugin" -a git_dir -a remote_url #{{{1
    command git --git-dir=$git_dir remote set-url origin $remote_url 2>/dev/null
    and \
        command git --git-dir=$git_dir fetch -q 2>/dev/null
end

function __fundle_install_plugin -d "install/update the given plugin" -a plugin -a git_url #{{{1
    if __fundle_no_git
        return 1
    end
...
function __fundle_show_doc_msg -d "show a link to fundle docs" #{{{1
...
function __fundle_load_plugin -a plugin -a path -a fundle_dir -a profile -d "load a plugin" #{{{1
    if begin
...
function __fundle_bind -d "set up bindings" #{{{1
...
function __fundle_init -d "initialize fundle" #{{{1
...
function __fundle_install -d "install plugin" #{{{1
    if test (count $__fundle_plugin_names) -eq 0
    ...
    for i in (__fundle_seq (count $__fundle_plugin_names))
        set -l name $__fundle_plugin_names[$i]
        set -l url $__fundle_plugin_urls[$i]
        if test -n $url
            __fundle_install_plugin $name $url $argv
        else
            continue
        end
    end
...
function __fundle_clean -d "cleans fundle directory" #{{{1
    set -l fundle_dir (__fundle_plugins_dir)
...
function __fundle_plugin --d 'add plugin to fundle' #{{{1
    set -l options 'u/url=' 'p/path=' 'c/cond=' 'l/local' 'h/help' 'd/debug'
    ...
    set -q _flag_local && set plugin_local 1
    ...
    if echo "$plugin_cond" | source
        ...
        # Process plugin
        test -z "$plugin_url" -a "$plugin_local" -eq 0
...
function __fundle_plug --d 'add plugin to fundle' #{{{1
    set -l help_txt "usage: fundle plugin NAME [[--url URL ] [--path PATH] [--cond CONDITION]]"
    test -z "$argv" && echo $help_txt && return 1
...
function __fundle_version -d "prints fundle version" #{{{1
    echo $__fundle_current_version
end

function __fundle_print_help -d "prints fundle help" #{{{1
    echo "usage: fundle (init | plugin | list | install | update | clean | self-update | version | help)"
end

function __fundle_list -d "list registered plugins" #{{{1
    if begin
            contains -- -s $argv
        ...
        for i in (__fundle_seq (count $__fundle_plugin_names))
            ...
            set -l url $__fundle_plugin_urls[$i]
            test -z $url
            and set url 'local plugin'
            echo {$name}\n\t{$url}
        end
    end
end

function fundle -d "run fundle" #{{{1
    if __fundle_no_git
        return 1
    end
